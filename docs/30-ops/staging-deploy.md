...
- **Local Postgres major version is 15; hosted is 17.** `supabase link` warns but the link succeeds; basic SQL works fine across both. Bump `supabase/config.toml` `[db].major_version = 17` if you start using PG-17-specific features locally.
...
- **Public map data is built at compile time, not runtime.** Astro is configured with `output: 'static'` — the homepage fetches `getCatalog()` during the build and bakes authors/countries into the HTML. New authors promoted via `/admin/promote` land in the DB immediately (admin inbox is dynamic), but **won't appear on the public map until the next Pages build**. To refresh after a promotion: Pages → Deployments → Retry deployment (or push any commit to `development`). The SSR migration in [40-phase2-backlog.md](../40-phase2-backlog.md) (`output: 'server'` + Cloudflare adapter) would remove this gap. A lighter alternative is a Supabase → Pages deploy-hook webhook that fires on `authors` insert.
